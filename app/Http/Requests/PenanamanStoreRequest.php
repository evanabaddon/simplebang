...
class PenanamanStoreRequest extends FormRequest
{
    /**
    ...
    {
        return [
            'judul' => 'required|string|min:4|max:100|unique:csr_out,judul',
            'tambang_id' => 'required|int|exists:tambang,id',
            'tanggal' => 'required|date',
            'luas' => 'required|int',
            'foto' => 'required|array',
            'foto.*' => 'required|image|mimes:jpg,jpeg,png,gif,svg',
            'bibit_id' => 'required|array',
            'bibit_id.*' => 'required|int|exists:bibit,id', 
            'csr_id' => 'required|array',
            'csr_id.*' => 'required|int|exists:csr,id', 
            'gudang_bibit' => 'required|array',
            'gudang_bibit.*' => 'required|int|exists:gudang_bibit,id', 
