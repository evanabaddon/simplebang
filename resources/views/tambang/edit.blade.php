               ...
               <!-- END: Top Bar -->
                ...
                <div class="pos intro-y grid grid-cols-12 gap-5 mt-5">
                    ...
                    <div class="intro-y col-span-12 lg:col-span-12">
                        <input type="text" id="nama" value="{{ $tambang->nama }}" name="nama" class="intro-y input input--lg w-full box pr-10 placeholder-theme-13" placeholder="Nama Tambang">
                                            ...
                                            <div class="mt-3">
                                                ...
                                                <select class="input w-full border mt-2" name="jenis_tambang_id">
                                                  @foreach($jenis_tambang as $each)
                                                    <option value="{{ $each->id }}" @if($each->id == $tambang->jenis_tambang_id) selected @endif>{{ $each->jenis }}</option>
                                                  @endforeach
                                                </select>
                                            </div>
                                            <div class="mt-3">
                                              <label for="">Alamat</label>
                                              <textarea name="alamat" class="input w-full border mt-2" placeholder="Alamat Tambang">{{ $tambang->alamat }}</textarea>
                                            </div>
                                            <div class="mt-3">
                                              <label for="">Luas</label>
                                              <input type="text" name="luas" class="input w-full border mt-2" value="{{ $tambang->luas }}" placeholder="Luas Tambang m²">
                                            </div>
                                            <div class="mt-3">
                                              <label for="">Status Lahan</label>
                                              <input type="text" name="status_lahan" class="input w-full border mt-2" value="{{ $tambang->status_lahan }}" placeholder="Status lahan">
                                            </div>
                                            <div class="mt-3">
                                                    ...
                                                    <div class="flex flex-wrap px-4" >
                                                      @foreach($foto_tambang as $each)
                                                        <div class="w-24 h-24 relative image-fit mb-5 mr-5 cursor-pointer zoom-in">
                                                            <img class="rounded-md" alt="{{ $tambang->nama }}" src="{{ $each->foto }}">
                                                            <a href="{{ route('tambang.delete.foto' , $each->id) }}"><div title="Remove this image?" class="tooltip w-5 h-5 flex items-center justify-center absolute rounded-full text-white bg-theme-6 right-0 top-0 -mr-2 -mt-2"> <i data-feather="x" class="w-4 h-4"></i> </div></a>
                                                        </div>
                                                      @endforeach
                                ...
                                <div class="tab-content__pane p-5" id="meta-title">
                                    <div class="border border-gray-200 rounded-md p-5 mt-5">
                                        <div class="mt-5">
                                            <div class="mt-3">
                                                <label>Nama</label>
                                                <input type="text" name="nama_pemilik" class="input w-full border mt-2" value="{{ $pemilik_tambang->nama }}" placeholder="Nama Pemilik">
                                            </div>
                                            <div class="mt-3">
                                              <label for="">Alamat</label>
                                              <textarea name="alamat_pemilik" class="input w-full border mt-2" placeholder="Alamat Pemilik">{{ $pemilik_tambang->alamat }}</textarea>
                                            </div>
                                            <div class="mt-3">
                                              <label for="">Email</label>
                                              <input type="email" name="email_pemilik" class="input w-full border mt-2" value="{{ $pemilik_tambang->email }}" placeholder="Email Pemilik">
                                            </div>
                                            <div class="mt-3">
                                                <label>Telepon</label>
                                                <input type="text" name="telepon_pemilik" class="input w-full border mt-2" value="{{ $pemilik_tambang->no_telepon }}" placeholder="Telepon Pemilik">
                                            </div>
                                    ...
                                    <div class="border border-gray-200 rounded-md p-5 mt-5">
                                        <div class="mt-5">
                                            <div class="mt-3">
                                                ...
                                                <input type="text" id="latitude" name="latitude" class="input w-full border mt-2" value="{{ explode(',' , $tambang->lokasi)[0] }}" placeholder="-x.xxxx">
                                            </div>
                                            <div class="mt-3">
                                                <label>Longitude</label>
                                                <input type="text" id="longitude" name="longitude" class="input w-full border mt-2" value="{{ explode(',' , $tambang->lokasi)[1] }}" placeholder="xxx.xxxx">
                                            </div>
...
function map_it(map , long , lat , judul){
...
$(document).ready(function(){
  ...
  var  map = new mapboxgl.Map({
              ...
              center: ["{{ explode(',' , $tambang->lokasi)[1] }}", "{{ explode(',' , $tambang->lokasi)[0] }}"], // starting position [lng, lat]
              zoom: 10 // starting zoom
            });
            map_it(map , '{{ explode(',' , $tambang->lokasi)[1] }}' , '{{ explode(',' , $tambang->lokasi)[0] }}' , '{{ $tambang->nama }}');
});
